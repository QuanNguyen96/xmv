...
class Admin_nestedset {
	 ...
	 */
	public function setControlParams($table_name = 'position', $left_column_name = 'lft', $right_column_name = 'rgt', $primary_key_column_name = 'id', $parent_column_name = 'parent', $text_column_name = 'level') {
    ...
    public function setPosition($position = 'content'){
	...
	*/
	...
	public function insertNewTree($extrafields = array()) {
		...
		$query = $this->position != null ? $this->db->where('position',$this->position)->get($this->table_name) : $this->db->get($this->table_name);
	...
	/**
     * Thêm 1 nút vào bên trái của nút
	...
	/**
     * Thêm 1 nút vào bên phải của nút
	...
	public function deleteTree() {
        ...
        {
            return $this->db->where('position',$this->position)->delete($this->table_name);
        }
        else
        {
    ...
    /** HÀM TỰ THÊM VÀO **/
    public function buildChosenMenu( $arrMenu = array(), $selected = null, $curent_id = null ){
        $html = '<select name="parent" class="select_menu_parent" id="select_menu_parent" onchange="check_position( \''.base_url('admin/menu/check_position').'\' )">';
        $html .= '<option value="">Chọn chuyên mục</option>';
        $strselected = '';
        if( $selected != null && $selected == 0 ){
             $strselected = 'selected=""';
        }
        $html .= '<option value="0" '.$strselected.'>Là chuyên mục gốc</option>';
        if( !empty( $arrMenu ) ){
               foreach( $arrMenu as $val ){
                        $slt = '';
                        if( $val['id'] ==  $selected ) $slt =' selected="" ';
                        $crdsb  = '';
                        if( $curent_id == $val['id'] ) $crdsb = ' disabled="true" ';
                        $str = '';
                        for( $i = 1; $i<= $val['level']; $i++ ){
                               $str.='&nbsp;&nbsp;';
                        }
                        $valu = $val['id'].'/'.$val['lft'].'/'.$val['rgt'].'/'.$val['level'];
                        $html.= '<option value="'.$valu.'" '.set_select( 'parent',$valu ).$slt.$crdsb.' >'.$str.$val['name'].'</option>'; 
               }
               unset( $arrMenu );
        }
        $html .='<select>';
