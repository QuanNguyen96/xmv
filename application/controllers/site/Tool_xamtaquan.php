<?php
class Tool_xamtaquan extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model(array('site/boituoivochong_model','site/article_model','site/xamtaquan_model'));
    ...
    public function action_xam_ta_quan(){
        $url    = $this->uri->uri_string();
        /** -------- input du lieu ----------- **/
        $this->load->library('site/vnkey');
        $data = array();
        if (isset($_SESSION['xinxamtaquan'])) {
            $data['xam'] = $this->session->userdata('xinxamtaquan');
        }
        ...
        $breadcrumb = array(
            ...
            1 => array(
                'name' => 'Xin xăm Tả Quân',
                'slug' => 'xin-xam-ta-quan',
            ),
        );
    ...
    function xinxam(){
        if ($this->input->is_ajax_request()) {
            $x = $this->xamtaquan_model->getAll();
            $que_so = random_element($x);
            $this->session->set_userdata('xinxamtaquan',$que_so['xam_so']);
            $this->output->set_output($this->session->userdata('xinxamtaquan'));
        }
        else
        {
    ...
    function xemnoidungque(){
        if ($this->input->is_ajax_request()) {
            $queso = $this->session->userdata('xinxamtaquan');
            $list = $this->xamtaquan_model->noi_dung_que($queso);
            $this->session->sess_destroy();
