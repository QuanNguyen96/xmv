...
class Menu extends CI_Controller {
        ...
        public $action_view = '';
        public $view        = 'index';
        public $default_module = array( array('id'=>0,'title' =>'Trang đơn','name'=>'single'), array('id'=>0,'title' =>'Liên kết','name'=>'hyperlink'));
        public function __construct(){
                             ...
                             $this->load->library( array('admin/admin_auth','admin/admin_pagination','admin/admin_nestedset', 'form_validation') );
        ...
        {                 $data['position']    = $this->get_position();
                          ...
                          if( !isset( $get['position'] ) )  $get['position']       = $data['position'][0]['name'];
                          $total_recod  = $this->menu_model->Db_count_index($get);
                          $list         = $this->menu_model->Db_get_index( $get, $this->limit );
                          ...
                          $data['list'] = $list;
                          $data['listsort'] = $this->menu_model->Db_get_listsort( $get );
                          $data['get']  = $get;
                          $data['success'] = $this->session->flashdata('success');
                          $data['error']   = $this->session->flashdata('error');
                          $data['total_recod'] = $total_recod;
                          $module         = $this->menu_model->db->select('id,name,title')->where('menu',1)->get('module')->result_array();
        ...
        public function add()
        {
                          ...
                          { 
                            if( $this->form_validation->run('add_menu') == true )
                            {
                                ...
                                $check_slug = $this->menu_model->db->where('slug',$slug)->get('menu')->row_array();
                                if( !empty( $check_slug ) )
                                {
                                ...
                                $insert['heading_h1']     = $post['heading_h1'];
                                $insert['description']  = $post['description'];
                                $insert['parent']       = $post['parent'];
                                $insert['state']        = $post['state'];
                                $insert['module']       = $post['module'];
                                if( isset($post['hyperlink']) || $post['hyperlink'] != null  ) $insert['hyperlink'] = $post['hyperlink'];
                                ...
                                {
                                    $parentNode = $this->menu_model->db->where('id',$insert['parent'])->get('menu')->row_array();
                                ...
                                if( isset( $post['position'] ) )
                                {
                                    ...
                                    $parent_position = $this->menu_model->db->where('menu_id',$post['parent'])->get('position')->result_array();
        ...
        public function edit()
        {
                          ...
                          if( $_POST )
                          {
                            if( $this->form_validation->run('edit_menu') == true )
                            {
                                ...
                                $update['heading_h1']     = $post['heading_h1'];
                                $update['keywords']     = $post['keywords'];
                                $update['description']  = $post['description'];
                                if( isset($post['hyperlink']) && $post['hyperlink'] != null  ) $update['hyperlink'] = $post['hyperlink'];
                                ...
                                if( $post['parent'] == $post['curent_parent'] ){
                                    ...
                                    $arr_menu_id_position = $this->menu_model->db->where('menu_id',$get['id'])->get('position')->result_array();
                                    ...
                                    {  
                                        ...
                                        {
                                            $this->admin_nestedset->setPosition($val);
                                            $parentNode = $this->menu_model->db->where('menu_id',$post['parent'])->where('position',$val)->get('position')->row_array();
                                            
                                            if( empty($parentNode) )
                                            {
                                              $newNode    = array('menu_id'=>$post['id'], 'parent'=>0, 'position'=>$val,'level'=>0);  
                                              
                                              $this->admin_nestedset->insertNewTree($newNode);
                                            }
                                            else
                                            {
                                               $newNode    = array('menu_id'=>$post['id'], 'parent'=>$parentNode['id'], 'position'=>$val,'level'=>$parentNode['level']+1);  
                          ...
                          {
                            $this->session->set_flashdata('error','<div class="error"><p>Bạn không thể sửa nội dung do người khác tạo.</p></div>');
                            redirect('admin/menu/index');
                          }
                          $action_view_image = base_url().'media/images/menu/';
                          $this->session->set_userdata('action_view_image', $action_view_image);
                          $data['category']      = $this->menu_model->Db_get_category_edit($data['item']['module']);
                          $cr_position           = $this->menu_model->db->select('position')->where('menu_id',$data['item']['id'])->get('position')->result_array();
                          $ar_position = array(); 
               ...
               if( $_POST ){
                  ...
                  foreach( $cid as $val )
                  {
                   ...
                   if( $created_by  ==  $this->admin_auth->get_colum('id') || $this->admin_auth->get_colum('manage') == 1 || $this->admin_auth->get_colum('email') == $this->admin_auth->get_root_email() )
                   {
                          ...
                          $nodePosition = $this->menu_model->db->where('menu_id',$val)->get('position')->result_array();
                  ...
                  redirect( base_url( 'admin'.'/'.$this->router->fetch_class().'/index?module='.$module.'&position'.$position ) );
              } 
        }
        ...
        public function ajax_get_menu_module()
        {
            ...
            {
                $module = $this->input->post('module');
                $data   = $this->menu_model->db->select('id,parent,level,name')->where('module',$module)->order_by('lft','ASC')->order_by('rgt','DESC')->get('menu')->result_array();
                //dequy($data,0,$result);
                ...
                if( !empty( $data ) )
                 {
                    foreach( $data as $val )
                    {
                       ...
                       $html.= '<option value="'.$val['id'].'">'.$char.' '.$val['name'].'</option>';
                    }
                }  
        ...
        public function ajax_get_position()
        {
            if( $_POST )
            {
                ...
                $data   = $this->menu_model->db->select('position')->where('menu_id',$id)->get('position')->result_array();
                $html = '';
                $cp   = array();
                if( !empty( $data ) )
                {
                ...
                foreach( $position as $val )
                {
                    ...
                    $html.='<li><input type="checkbox" name="position[]" value="'.$val['name'].'" '.$ck.' >&nbsp<span>'.ucfirst($val['title']).'</span>';
                } 
                $arr  = array( 'result'=> $html ); 
                $this->output
                     ->set_content_type('application/json')
        ...
        public function ajax_sort_menu()
        {
            if( $_POST )
            {
                ...
                $curent_id = $this->input->post('id');
                $position  = $this->input->post('position');
                $target    = $this->menu_model->db->where('id',$parent)->where('position',$position)->get('position')->row_array();
                $node      = $this->menu_model->db->where('id',$curent_id)->where('position',$position)->get('position')->row_array();
                $this->admin_nestedset->setControlParams();
                ...
                switch($sort)
                {
                    case 'pre' :
                        ...
                        $this->menu_model->db->where('id',$node['id'])->update('position',array('level'=>$node['level']));
        ...
        public function update_level($node=null)
        {
            ...
            $node = $this->menu_model->db->where('id',$node['id'])->get('position')->row_array();
...
//            exit(); 
        ...
        {
            $mang = array(
                           array( 'name'=>'top', 'title' => 'Phía trên' ),
                           array( 'name'=>'right', 'title' => 'bên phải' ),
                           array( 'name'=>'content', 'title' => 'trung tâm' ),
                           array( 'name'=>'footer', 'title' => 'chân trang' ),
