...
class Tuvingay_nguhanh extends CI_Controller {
        public $limit = 50;
        ...
        public function __construct(){
                             ...
                             $this->load->model( array('admin/tuvingay_nguhanh_model') );
        ...
        {                   
                          ...
                          $total_recod  = $this->tuvingay_nguhanh_model->Db_count_index($get);
                          $list         = $this->tuvingay_nguhanh_model->Db_get_index( $get, $this->limit );
        ...
        public function edit()
        {
                          ...
                          if( $_POST )
                          {
                            //if( $this->form_validation->run('edit_tuvingay_nguhanh') == true )
                            //{
                                $post = $this->input->post();
                                $update['y_nghia'] = serialize($post['y_nghia']);
                                $this->tuvingay_nguhanh_model->db->where('id',$post['id'])->update('tvn_ngu_hanh',$update);
                                ...
                                else
                                {
                                    ...
                                    redirect( base_url('admin/tuvingay_nguhanh/index?page='.$page) );
                                }
                            //}
                            //else
                            //{
                          ...
                          $data['item']         = $this->tuvingay_nguhanh_model->Db_get_edit( $get );
                          if( empty($data['item']) ) show_404();
                          $action_view_image = base_url().'media/images/tuvingay_nguhanh/';
        ...
        public function delete()
        {
            if( $_POST && !empty( $_POST['cid'] ) )
            {
                ...
                // $this->tuvingay_nguhanh_model->db->where_in('id',$cid)->delete('tuvingay_nguhanh');
                // $this->session->set_flashdata('success','<div class="success"><p>Xóa thành công!</p></div>');
                redirect( base_url('admin/tuvingay_nguhanh/index') );
            }
        }
