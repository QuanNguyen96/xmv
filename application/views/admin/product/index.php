...
$param['order_id'] = 'created_date';
$sort_link_created_date  = current_url().'?'.http_build_query($param); 
$param['order_id'] = 'orders';
$sort_link_orders = current_url().'?'.http_build_query($param); 
?>
<article>
...
<header class="header_control">
  ...
  <ul>
     <li><a href="<?php echo base_url('admin/product/add');?>" class="button">Thêm</a></li>
     <li><a href="#" class="button" onclick="submit_form('<?php echo base_url('admin/product/delete');?>');">Xóa</a></li>
  </ul>
</header>
<div class="fillter">
   <form name="fillter" action="" method="get"> 
     <ul>
        ...
        <li><a href="<?php echo base_url('admin/product/index');?>" class="button">Hủy</a></li>
     </ul>
    </form>  
  </div>
      ...
      <table class="table">
         <thead>
            <tr>
                ...
                <th><a href="<?php echo $sort_link_orders; ?>">Thứ tự&nbsp;<i class="fa fa-sort-<?php echo $param['order'];?>" aria-hidden="true"></i></a></th>
                <th><a href="<?php echo $sort_link_created_date; ?>">Ngày tạo&nbsp;<i class="fa fa-sort-<?php echo $param['order'];?>" aria-hidden="true"></i></a></th>
         ...
         <tbody>
            <?php if( !empty($list) ):
                   foreach( $list as $val ):
                   $edit_link = base_url('admin/product/edit?id='.$val['id'].'&page='.$get['page']);
                   $edit_name = $val['state'] == 0 ? $val['name'].'<i class="nhap">(bản nháp)</i>' : $val['name']; 
                   $parent_link = base_url( 'admin/product/index?category='.$val['parent'] );
            ?>
             <tr>
