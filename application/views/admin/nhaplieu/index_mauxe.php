...
<header class="header_control">
  <label>Quản lý nội dung màu xe hợp mệnh</label>
  <ul>
     <li><a href="<?php echo base_url('admin/nhaplieu/add_mauxe');?>" class="button">Thêm</a></li>
     <li><a href="#" class="button" onclick="submit_form('<?php echo base_url('admin/nhaplieu/delete_mauxe');?>');">Xóa</a></li>
  </ul>
</header>
<div class="fillter">
   <form name="fillter" action="" method="get"> 
     <ul>
        ...
        <li><a href="<?php echo base_url('admin/nhaplieu/index_mauxe');?>" class="button">Hủy</a></li>
     </ul>
    </form>  
  </div>
      ...
      <table class="table">
         <thead>
            <tr>
                ...
                <th><span>Mệnh</span></th>
            </tr>
         </thead>
         <tbody>
            <?php if( !empty($list) ):
                   foreach( $list as $val ):
                   $edit_link = base_url('admin/nhaplieu/edit_mauxe?id='.$val['id'].'&page='.$get['page']);
            ?>
             <tr>
                <td><span><input type="checkbox" name="cid[]" value="<?php echo $val['id']; ?>" class="cid" /></span></td>
                <td><a href="<?php echo $edit_link;?>"><?php echo $val['menh_text']; ?></a></td>
             </tr>
