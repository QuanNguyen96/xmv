...
<header class="header_control">
  <label>Quản lý nội dung bói chỉ tay</label>
  <ul>
     <li><a href="<?php echo base_url('admin/nhaplieu/add_boichitay');?>" class="button">Thêm</a></li>
     <li><a href="#" class="button" onclick="submit_form('<?php echo base_url('admin/nhaplieu/delete_boichitay');?>');">Xóa</a></li>
  </ul>
</header>
<div class="fillter">
   <form name="fillter" action="" method="get"> 
     <ul>
        ...
        <li><a href="<?php echo base_url('admin/nhaplieu/index_boichitay');?>" class="button">Hủy</a></li>
     </ul>
    </form>  
  </div>
  <form name="adminForm" id="adminForm" action="" method="post">
      <table class="table">
         ...
         <tbody>
            <?php if( !empty($list) ):
                   foreach( $list as $val ):
                   $edit_link = base_url('admin/nhaplieu/edit_boichitay?id='.$val['id'].'&page='.$get['page']);
            ?>
             <tr>
                <td><span><input type="checkbox" name="cid[]" value="<?php echo $val['id']; ?>" class="cid" /></span></td>
                <td><a href="<?php echo $edit_link;?>"><?php echo $val['name_text']; ?></a></td>
                <td><?php echo $val['name_slug']; ?></td>
             </tr>
