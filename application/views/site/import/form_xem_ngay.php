<?php
$link_ngay_tot = isset($input['link_ngay_tot']) ? $input['link_ngay_tot'] : '';
?>
<?php if ($this->mobile_detect->isMobile()): ?>
	<div class="boxFormXemNgay inner_box_red ip_form_xem_ngay box_cuoihoi_fix">
		<form class="form_vien" name="ngaytotxau_tuoicanchi" method="post" onsubmit="submit_formXemngay_totxau_theotuoi('<?php echo $link_ngay_tot; ?>');">
            <div class="txt_1xnt">
					...
					<div class="row">
						<div class="col-sm-12 col-xs-12">
							<select name="nam_sinh">
								<?php 
                                $cr_sl = isset($canchislug) ? $canchislug : 'nham-than';
								foreach (list_age_text() as $key => $value):
									$selected = $cr_sl == $key ? 'selected=""' : '';
								 ?>
...
<?php endif ?>
<script>
	function submit_formXemngay_totxau_theotuoi(cong_cu) {
		if(cong_cu != '')
			cong_cu = '-'+cong_cu;
		var frm    = document.ngaytotxau_tuoicanchi;
		var canchi = frm.nam_sinh.value;
     	var link   = 'xem-ngay-tot'+cong_cu+'-tuoi-' + canchi + '.html';
