<?php $gioitinh = isset($gioitinh) ? $gioitinh : 'Nam'; ?>
			...
			<div class="col-sm-4 col-xs-4">
				<select required="" name="namsinh" id="" class="input_thanhchi">
					<option value="" selected="" disabled="">Năm sinh</option>
						<?php 
	                        foreach( namsinh_uutien() as $key => $value ){ 
		                            $slt = $value == isset($namsinh)? ($namsinh ? 'selected=""' : '') : '';
		                            if (isset($menh_nguoi)) {
		                            	$get_birth_by_menh = get_birth_by_menh($menh_nguoi);
		                            	if (in_array($value, $get_birth_by_menh)) {
		                            		echo '<option value="'.$value.'" '.$slt.' >'.$value.' ('.namsinh_menh($value).')</option>';
		                            	}
		                            }
		                            else{
		                            	echo '<option value="'.$value.'" '.$slt.' >'.$value.'</option>';
		                            }
	...
	<?php else : ?>
		<div class="row mar-bot">
			<div class="col-sm-12 col-xs-12">
				<input type="tel" name="sosim" class="input_thanhchi" placeholder="Số điện thoại">
			</div>
		</div>
		<div class="row mar-bot">
			<div class="col-sm-4 col-xs-4">
			...
			<div class="col-sm-4 col-xs-4">
				<select name="namsinh" id="" class="input_thanhchi">
					<option value="" selected="" disabled="">Năm sinh</option>
						<?php 
	                        for( $i = 1950; $i<= 2025; $i++ ){
		...
		<div class="row mar-bot">
			...
			<div class="col-sm-5 col-xs-5 text-center" style="line-height: 38px;color: #fff;">
				<input type="radio" value="Nam" name="gioitinh" id="gt_nam" <?php echo ($gioitinh == 'Nam') ? 'checked=""' : ''?>><label for="gt_nam">&nbsp;&nbsp;Nam</label>
				<input type="radio" value="Nữ" name="gioitinh" id="gt_nu"  <?php echo ($gioitinh == 'Nũ') ? 'checked=""' : ''?>><label for="gt_nu">&nbsp;&nbsp;Nữ</label>
			</div>
		</div>
	<?php endif; ?>
