...
class Menu_model extends CI_Model {
         ...
         public function Db_count_menu( $get )
         ...
         public function Db_get_menu( $get, $limit )
         { 
                      $offset = ( (int)$get['page'] - 1 ) * $limit;
                      $this->db->select('m.id,m.name, m.created_date, m.state, u.fullname as created_by, m.level, m.parent');
                      ...
                      if($get['key'] != null )
                      $this->db->like('m.name',$get['key']); 
                      $this->db->order_by( 'm.lft', 'ASC' );
                      $this->db->order_by( 'm.rgt', 'DESC' );
                      $this->db->limit($limit, $offset); 
                      $query = $this->db->get(); 
                      return $query->result_array();
         }
         
         public function Db_count_menu_position( $get )
         {         if( isset( $get['key'] ) && $get['key'] != null )
                       $this->db->like('m.name',$get['key']);
         ...
         public function Db_get_menu_position( $get, $limit )
         { 
                      $offset = ( (int)$get['page'] - 1 ) * $limit;
         ...
         public function Db_get_menu_position_sort( $get )
         { 
                      $this->db->select('m.id,m.name,p.level, p.id as position_id, p.parent');
                      $this->db->from('menu as m');
                      $this->db->join('position as p', 'p.menu_id = m.id');
